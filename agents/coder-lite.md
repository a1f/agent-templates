...
- **Conform gate — never skip.** Before staging, re-read the language rule and check **every line
  ...
  one sentence for an interface, one line a reader needs inline, and the reasoning of a slice in
  its commit message, never in the code. A **black-letter** violation (one the rule states
