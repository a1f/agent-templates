...
`target_cwd` and return `test_file` relative to `target_cwd`. Read every rule path the dispatch
passed, in full — typically `tdd.md`, the **language rule** (`python.md`/`typescript.md`/`rust.md`),
and `design-principles.md`. Your test is production-grade code: hold it to the **full** language rule — types,
...
2. **Write one test.**
   ...
   - Write it to the language rule, same as production code (see Inputs), so the reviewer holds the
     test to it too.
...
3. **Run it and confirm RED for the right reason.** It must fail on the **assertion** (or a
   ...
   `status: error`. Paste the real failure output. If the test instead **passes** on its first run,
   there is no RED — the behavior already exists or the assertion is vacuous; do not weaken or
   contort it into a failure, return `status: error` with `right_reason: false` and a
   `right_reason_note` saying so, so the architect can re-scope.
...
- **One behavior, one test** — batching is the horizontal-slice anti-pattern.
- **No production code** — leave the implementation for the coder (a missing symbol the GREEN step
  will add is a valid RED, per step 3).
